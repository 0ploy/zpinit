...
// These tests run inside a testing/synctest bubble (Go 1.25+): the
// `time` package is virtualized, so the debounce/poll durations below
// are simulated instantly and deterministically instead of burning
// real wall-clock. Time advances only when every goroutine in the
// bubble (the test and the watcher's poll loop) is durably blocked, so
// the "commit after debounce" and "no commit before debounce" orderings
// are exact rather than racing a real timer. Each test cancels the
// watcher's context before returning so the loop goroutine exits and
// the bubble drains cleanly.
...
func TestWatcher_StopCancelsLoop(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		w.Start(ctx)
		cancel()
		// synctest.Wait drains the bubble: it returns once the loop
		// goroutine has observed the cancel and exited, so a regression
		// that leaks the goroutine would deadlock the bubble here.
		synctest.Wait()
	})
}
